...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      <h2 className="text-3xl font-extrabold text-textPrimary mb-8">Dashboard Overview</h2>

      {lowStockAlerts.length > 0 && (
        <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 p-4 rounded-lg shadow-md mb-8" role="alert">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-card p-6 rounded-lg shadow-md border-l-4 border-primary">
          <h3 className="text-lg font-semibold text-textSecondary mb-2">Total Items</h3>
          <p className="text-4xl font-bold text-textPrimary">{summary?.totalItems ?? 0}</p>
        </div>
        <div className="bg-card p-6 rounded-lg shadow-md border-l-4 border-secondary">
          <h3 className="text-lg font-semibold text-textSecondary mb-2">Total Vehicles</h3>
          <p className="text-4xl font-bold text-textPrimary">{summary?.totalVehicles ?? 0}</p>
        </div>
        <div className="bg-card p-6 rounded-lg shadow-md border-l-4 border-accent">
          <h3 className="text-lg font-semibold text-textSecondary mb-2">Total Stock Quantity</h3>
          <p className="text-4xl font-bold text-textPrimary">{summary?.totalStockQuantity ?? 0}</p>
        </div>
      </div>

      <div className="bg-card p-6 rounded-lg shadow-md mb-8">
        <h3 className="text-xl font-bold text-textPrimary mb-4">Recent Transactions</h3>
        {recentTransactions.length > 0 ? (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {recentTransactions.map((transaction) => (
                  <tr key={transaction.id}>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{transaction.itemName}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        transaction.type === TransactionType.STOCK_IN ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                      }`}>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{transaction.vehicleName || 'N/A'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <p className="text-textSecondary">No recent transactions to display.</p>
        )}
      </div>

      {summary && summary.totalItems === 0 && summary.totalVehicles === 0 && (
        <div className="text-center p-6 bg-yellow-50 rounded-lg border border-yellow-200">
          <p className="text-lg text-yellow-800 mb-4">It looks like your inventory is empty!</p>
