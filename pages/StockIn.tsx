...
export const StockIn: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to fetch data for stock in:', error);
  ...
  const handleAddItemToList = () => {
    ...
    if (transactionItems.some(item => item.itemId === currentItem.itemId)) {
      errors.itemId = 'This item is already in the list. Remove it first to change the quantity.';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast(`${transactionItems.length} stock in transaction(s) recorded successfully!`, 'success');
      setTransactionItems([]);
      setCurrentItem({ ...initialCurrentItemState, itemId: items[0]?.id || '' });
      setTransactionDetails(prev => ({ ...prev, date: formatDate(new Date()) }));
      await fetchData(); // Refresh data
    } catch (error) {
      console.error('Failed to record stock in transaction:', error);
